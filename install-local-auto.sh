...
# MoxNAS Local Automated Installation Script
# This script installs MoxNAS locally without LXC container
...
Network Attached Storage - Local Installation
Version: 2.0.0 Auto
EOF
}
...
create_moxnas_user() {
    msg_info "Creating MoxNAS user and directories..."
...
setup_application() {
    msg_info "Setting up MoxNAS application..."
    
    cd "$MOXNAS_HOME"
    
    # Copy application files if not already there
    if [ ! -f "$MOXNAS_HOME/app/__init__.py" ]; then
        cp -r /home/wassim/Documents/MoxNAS/* "$MOXNAS_HOME/"
        chown -R "$MOXNAS_USER:$MOXNAS_USER" "$MOXNAS_HOME"
    fi
...
main() {
    ...
    msg_info "Starting automated MoxNAS installation..."
