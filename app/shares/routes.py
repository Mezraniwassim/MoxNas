...
from app.models import (Share, Dataset, ShareProtocol, ShareStatus, SystemLog, 
                       LogLevel, UserRole)
from app import db
...
def index():
    ...
    return render_template('shares/index.html', 
                         shares=shares,
...
def create():
    ...
    if request.method == 'POST':
        try:
            ...
            if protocol_manager:
                success, message = protocol_manager.create_smb_share(share) if protocol == ShareProtocol.SMB else \
                                  protocol_manager.create_nfs_share(share) if protocol == ShareProtocol.NFS else \
                                  protocol_manager.create_ftp_share(share)
...
def detail(share_id):
    ...
    return render_template('shares/detail.html', 
                         share=share, 
...
def toggle(share_id):
    ...
    try:
        ...
        if share.status == ShareStatus.ACTIVE:
            # Disable share
            success, message = protocol_manager.delete_smb_share(share) if share.protocol == ShareProtocol.SMB else \
                              protocol_manager.delete_nfs_share(share) if share.protocol == ShareProtocol.NFS else \
                              protocol_manager.delete_ftp_share(share)
            ...
            # Enable share
            success, message = protocol_manager.create_smb_share(share) if share.protocol == ShareProtocol.SMB else \
                              protocol_manager.create_nfs_share(share) if share.protocol == ShareProtocol.NFS else \
                              protocol_manager.create_ftp_share(share)
...
def delete(share_id):
    ...
    try:
        ...
        return jsonify({
            'success': True, 
            'message': f'Share "{share_name}" deleted successfully'
        })
