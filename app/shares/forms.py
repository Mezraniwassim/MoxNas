...
class ShareProtocolForm(FlaskForm):
    ...
    # SMB specific
    smb_workgroup = StringField("Workgroup", validators=[Optional()], default="WORKGROUP")
    smb_security = SelectField(
        "Security", choices=[("user", "User Level"), ("share", "Share Level")], default="user"
    )
    ...
    # FTP specific
    ftp_passive_port_range = StringField(
        "Passive Port Range",
        validators=[Optional()],
        default="10000-10100",
        render_kw={"placeholder": "10000-10100"},
    )
    ftp_max_clients = IntegerField(
        "Max Clients", validators=[Optional(), NumberRange(min=1, max=100)], default=10
    )
