    ...
    def check_service_health(self, service_name: str) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if status.status == ServiceStatus.FAILED:
                ...
                if success:
                    error_logs = [log for log in logs if 'error' in log.lower() or 'fail' in log.lower()]
                    if error_logs:
            ...
            if status.memory_usage and status.memory_usage > 1024 * 1024 * 1024:  # 1GB
                ...
                health_info['recommendations'].append('Monitor memory usage and consider service restart')
