...
class MoxNAS {
    ...
    setupEventListeners() {
        // Navigation links
        document.querySelectorAll('.truenas-nav-link').forEach(link => {
     ...
     */
    toggleStorageView(viewType) {
        if (window.storageComponents) {
            window.storageComponents.toggleView(viewType);
        }
    }
    ...
    /**
     * Toggle shares view between table and cards
     */
    toggleSharesView(viewType) {
        const tableContainer = document.getElementById('shares-table');
        const cardsContainer = document.getElementById('shares-cards');
        const tableViewBtn = document.getElementById('shares-table-view');
        const cardViewBtn = document.getElementById('shares-card-view');
    ...
    /**
     * Switch tabs for settings
     */
    switchTab(tabId) {
        document.querySelectorAll('.truenas-tab-content').forEach(tab => {
            tab.classList.remove('active');
        });
        document.querySelectorAll('.truenas-tab-button').forEach(button => {
            button.classList.remove('active');
        });
    ...
    async toggleService(serviceName) {
        console.log(`Toggling ${serviceName} service`);
        this.showAlert('info', `${serviceName.toUpperCase()} service toggle requested`);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.storageComponents) {
        window.storageComponents.init();
    }
    
    if (window.networkComponents) {
        window.networkComponents.init();
    }
});
