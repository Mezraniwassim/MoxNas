  ...
  const loadData = async () => {
    try {
      ...
      if (sharesResponse.status === 'fulfilled') {
        setShares(sharesResponse.value.data.results || sharesResponse.value.data);
      } else {
  ...
  const handleSaveShare = async (formData) => {
    try {
      ...
      if (!formData.name || !formData.path) {
        alert('Share name and path are required');
        return;
      }
      ...
      // Ensure path starts with /mnt/storage if not absolute
      if (!formData.path.startsWith('/')) {
        formData.path = `/mnt/storage/${formData.path}`;
      }
      
      // Validate path is in allowed directory
      const allowedBases = ['/mnt/storage', '/opt/moxnas/storage', '/home', '/tmp/moxnas'];
      if (!allowedBases.some(base => formData.path.startsWith(base))) {
        if (window.confirm('Path is outside allowed directories. Use /mnt/storage instead?')) {
          formData.path = `/mnt/storage/${formData.name}`;
        } else {
          return;
        }
      }
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  {!editingShare && shares.length > 0 && (
                    <small style={{ 
                      ...
                      Existing shares: {shares.map(s => s.name).join(', ')}
                    </small>
                  )}
                </div>
                <div style={{ marginBottom: '1rem' }}>
