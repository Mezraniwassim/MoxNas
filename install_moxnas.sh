    ...
    pct exec "$CONTAINER_ID" -- bash -c "
        ...
        if command -v node >/dev/null 2>&1; then
            ...
            if [ \"\$MAJOR_VERSION\" -lt 14 ]; then
                ...
                echo 'Node.js version \$NODE_VERSION is acceptable'
            fi
        else
...
install_moxnas() {
    ...
    pct exec "$CONTAINER_ID" -- bash -c "
        ...
        # Set optimal Node.js memory options for LXC containers
        export NODE_OPTIONS=\"--max-old-space-size=1536 --max-semi-space-size=64\"
        export GENERATE_SOURCEMAP=false
        export CI=false
        
        # Build frontend with comprehensive fallback strategy
        echo \"Building frontend with memory optimization...\"
        timeout 600 npm run build || {
            echo \"Frontend build failed or timed out, trying with reduced memory...\"
            export NODE_OPTIONS=\"--max-old-space-size=768\"
            timeout 300 npm run build || {
                echo \"Second build attempt failed, trying development build...\"
                npm run build:dev 2>/dev/null || {
                    echo \"Development build not available, creating minimal build...\"
                    
                    # Create minimal build structure
                    mkdir -p build/static/{css,js}
                    
                    # Create basic index.html
                    cat > build/index.html << 'BUILD_EOF'
<!DOCTYPE html>
<html lang=\"en\">
<head>
    <meta charset=\"utf-8\" />
...
<body>
    <div class=\"container\">
        <h1>🗄️ MoxNAS - Network Attached Storage</h1>
        <div class=\"loading\">
            <p>Welcome to MoxNAS! The React frontend is being prepared.</p>
            <p>Please use the following interfaces:</p>
            <a href=\"/admin/\" class=\"btn\">Django Admin Interface</a>
            <a href=\"/api/\" class=\"btn\">REST API Documentation</a>
            <p><small>Container IP: <span id=\"ip\">Loading...</span></small></p>
        </div>
    </div>
    <script>
        // Get container IP
        fetch('/api/system/info/').then(r => r.json()).then(data => {
            document.getElementById('ip').textContent = data.hostname || 'Unknown';
        }).catch(() => {
            document.getElementById('ip').textContent = 'Check container status';
        });
        
        // Auto-redirect to admin after delay
        setTimeout(() => {
            if (confirm('Redirect to Django Admin Interface?')) {
...
show_info() {
    ...
    echo "  Web test: curl -I http://\$CONTAINER_IP"
    echo ""
    
    # Final verification test
    log_info "Performing final web interface test..."
    if curl -I "http://\$CONTAINER_IP" &> /dev/null; then
        log_success "✅ MoxNAS web interface is accessible!"
        log_success "🌐 Open: http://\$CONTAINER_IP"
